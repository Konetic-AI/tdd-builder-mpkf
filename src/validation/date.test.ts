/**
 * @fileoverview Comprehensive unit tests for ISO-8601 date validation
...
describe('ISO-8601 Date Validation', () => {
  describe('validateIS8601Date', () => {
    ...
    describe('Input Validation', () => {
      test('should reject null input', () => {
        const result = validateIS8601Date(null as any);
        expect(result.isValid).toBe(false);
        expect(result.error).toContain('Date string must be a non-empty string');
      });

      test('should reject undefined input', () => {
        const result = validateIS8601Date(undefined as any);
        expect(result.isValid).toBe(false);
        expect(result.error).toContain('Date string must be a non-empty string');
      });
      ...
      test('should reject non-string input', () => {
        const result = validateIS8601Date(123 as any);
        expect(result.isValid).toBe(false);
        expect(result.error).toContain('Date string must be a non-empty string');
      });
    });
  ...
  describe('detectIS8601Format', () => {
    ...
    test('should return null for invalid formats', () => {
      ...
      expect(detectIS8601Format(null as any)).toBeNull();
    });
  });
